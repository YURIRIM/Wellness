...
--
--create user ROUTIN identified by ROUTINE;
...
COMMENT ON COLUMN CHALLENGE_CATEGORY.EMBEDDING IS '임베딩 벡터 JSON';
...
--챌린지 종료 시 실패 처리
CREATE OR REPLACE TRIGGER TRG_CHALLENGE_AUTO_FAIL
AFTER UPDATE OF STATUS ON CHALLENGE
FOR EACH ROW
WHEN (NEW.STATUS = 'N' AND OLD.STATUS <> 'N')
BEGIN
  UPDATE CHALLENGE_PARTICIPATION
     SET STATUS = 'F'
   WHERE CHAL_NO = :NEW.CHAL_NO;
END;
/
...
CREATE TABLE VIDEO_CALL (
    ...
    CREATE_DATE          TIMESTAMP,
    IS_RESTRICT          VARCHAR2(1),
    STATUS               VARCHAR2(1),

    CONSTRAINT PK_ROOM_UUID PRIMARY KEY (ROOM_UUID),
    CONSTRAINT FK_VIDEO_CALL_CHAL_NO
        FOREIGN KEY (CHAL_NO) REFERENCES CHALLENGE(CHAL_NO),
    CONSTRAINT UK_VIDEO_CALL_CHAL_NO UNIQUE (CHAL_NO),
    CONSTRAINT NN_ROOM_UUID	 CHECK (ROOM_UUID IS NOT NULL),
    CONSTRAINT NN_ROOM_NAME    CHECK (ROOM_NAME IS NOT NULL),
    CONSTRAINT CK_IS_RESTRICT     CHECK (STATUS IN ('N', 'Y', 'S')),
    CONSTRAINT CK_VIDEO_CALL_STATUS     CHECK (STATUS IN ('Y', 'N'))
    
);
COMMENT ON TABLE VIDEO_CALL IS '화상회의';
COMMENT ON COLUMN VIDEO_CALL.ROOM_UUID IS '회의실 UUID';
COMMENT ON COLUMN VIDEO_CALL.ROOM_NAME IS '이름';
COMMENT ON COLUMN VIDEO_CALL.CREATE_DATE IS '생성일';
COMMENT ON COLUMN VIDEO_CALL.IS_RESTRICT IS '참여 제한(N:없음, Y:참여자만, S:참여자 및 성공한 사람)';
