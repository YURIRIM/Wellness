...
CREATE TABLE CHALLENGE (
    ...
    USER_NO           NUMBER        CONSTRAINT FK_CHALLENGE_USER REFERENCES USER(USER_NO),
    ...
    THUMBNAIL         BLOB          CONSTRAINT NN_CHALLENGE_THUMBNAIL NOT NULL,
    VERIFY_CYCLE	    NUMBER(3),
    CREATE_DATE       TIMESTAMP     CONSTRAINT NN_CHALLENGE_CREATE_DATE NOT NULL,
    START_DATE        TIMESTAMP,
    END_DATE          TIMESTAMP,
    STATUS            VARCHAR2(1)   CONSTRAINT CK_CHALLENGE_STATUS CHECK (STATUS IN ('Y','N','D')) DEFAULT 'Y',
...
CREATE TABLE CHALLENGE_PARTICIPATION (
    CHAL_NO           NUMBER        CONSTRAINT FK_USER_CHAL_NO REFERENCES CHALLENGE(CHAL_NO),
    USER_NO           NUMBER        CONSTRAINT FK_CHALLENGE_USER REFERENCES USER(USER_NO),
    STATUS            VARCHAR2(1)   CONSTRAINT CK_CHALLENGE_PARTICIPATION_STATUS CHECK (STATUS IN ('Y','S','F')),
...
CREATE TABLE CHALLENGE_COMMENT (
    ...
    USER_NO           NUMBER        CONSTRAINT FK_COMMENT_USER REFERENCES USER(USER_NO),
    ...
    REPLY             VARCHAR2(3300),
    STATUS            VARCHAR2(1)   CONSTRAINT CK_COMMENT_STATUS CHECK (STATUS IN ('Y','N')) DEFAULT 'Y'
);
...
CREATE TABLE ATTACHMENT (
    ...
    REF_NO          NUMBER        CONSTRAINT NN_REF_NO NOT NULL,
    REF_TABLE		VARCHAR2(1)   CONSTRAINT CK_ATTACHMENT_STATUS CHECK (STATUS IN ('C','T','R')),
...
CREATE TABLE PROFILE (
    USER_NO           NUMBER        CONSTRAINT FK_PROFILE_USER REFERENCES USER(USER_NO) UNIQUE,
    PICTUR            BLOB,
    BIO               VARCHAR2(330),
    IS_OPEN           VARCHAR2(1)   CONSTRAINT CK_COMMENT_STATUS CHECK (STATUS IN ('Y','N','A'))
);
COMMENT ON TABLE PROFILE IS '프로필';
COMMENT ON COLUMN PROFILE.USER_NO IS '대상 회원';
COMMENT ON COLUMN PROFILE.PICTUR IS '프로필 사진';
COMMENT ON COLUMN PROFILE.BIO IS '자기소개';
COMMENT ON COLUMN PROFILE.IS_OPEN IS '프로필 공개여부 (Y:공개, N:비공개 - 프로필/닉네임/자기소개만 공개, A:익명)';
