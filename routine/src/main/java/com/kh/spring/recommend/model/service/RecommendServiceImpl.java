...
public class RecommendServiceImpl implements RecommendService {
	...
	public Weather getWeatherInfo(Location location) {
		...
		try {
			...
			String weatherUrl = String.format(
					"%s/getVilageFcst?serviceKey=%s&pageNo=1&numOfRows=100&dataType=JSON&base_date=%s&base_time=%s&nx=%d&ny=%d",
							...
							if (item != null) {
								...
								for (Map<String, String> itemData : item) {
									...
									if (fcstValue != null) {
										...
										case "TMP":
											newWeather.setTemperature((int) Double.parseDouble(fcstValue));
											break;

										case "REH":
											newWeather.setHumidity((int) Double.parseDouble(fcstValue));
											break;

										case "POP":
											newWeather.setRainProbability((int) Double.parseDouble(fcstValue));
											break;

										case "PTY":

											if ("1".equals(fcstValue) || "2".equals(fcstValue)
													|| "4".equals(fcstValue)) {
										...
										case "SKY":
											...
											if (newWeather.getWeatherCondition() == null
													|| "알 수 없음".equals(newWeather.getWeatherCondition())) {

												if ("1".equals(fcstValue))
													newWeather.setWeatherCondition("맑음");

												else if ("3".equals(fcstValue))
													newWeather.setWeatherCondition("구름많음");

												else if ("4".equals(fcstValue))
													newWeather.setWeatherCondition("흐림");
			...
			if (airResponseBody != null && airResponseBody.containsKey("response")) {
				...
				if (responseMap.containsKey("header")) {
					...
					if (!"00".equals(resultCode)) {
						...
						if (bodyMap != null && bodyMap.containsKey("items") && bodyMap.get("totalCount") != null
								&& (Integer) bodyMap.get("totalCount") > 0) {
							...
							if (!item.isEmpty()) {
								...
								newWeather.setPm10(
										pm10Value != null && !"-".equals(pm10Value) ? Integer.parseInt(pm10Value)
												: null);

								newWeather.setAirGrade(airGradeValue != null && !"-".equals(airGradeValue)
										? convertAirGradeToText(airGradeValue)
										: "알 수 없음");
	...
	public List<Recommend> getRecommendations(Weather weather) {
		...
		if (weather != null && weather.getTemperature() != null && weather.getPm10() != null
				&& weather.getRainProbability() != null) {
